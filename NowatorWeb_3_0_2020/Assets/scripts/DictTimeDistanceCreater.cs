...
public class DictTimeDistanceCreater : MonoBehaviour
{
    ...
    void startTest(){
        if ( (basisVectorMuvement*now_setup).magnitude <= muvementVector.magnitude){
            ...
            print("ended work");
            // print(result);
            gameObject.GetComponent<TestingNeednessMuvementMapScr>().WorkWithFile(_rb.mass, flyScr.stiffness_coefficient, result);
            this.enabled = false;
        }
    }

    void TestingNeednessMuvementMap(float mass, float coefficient){

    }

    void CreateMuvementMap(float mass, float coefficient, string data){
        // var path = "Assets/scripts/maps/muvement_" + mass.ToString().Replace(",", "F") + "_" + coefficient.ToString().Replace(",", ".") + ".cs";
